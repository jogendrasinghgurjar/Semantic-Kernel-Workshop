...
namespace SKWorkshop.Plugin.CustomPlugin
{
    public class VehicleParking
    {
        IKernel _kernel;
        public VehicleParking(IKernel kernel)
        {
            this._kernel = kernel;
        }

        [SKFunction, Description("fetch available parking slot")]
        [SKParameter("fuelType", "The fuel type of vehicle")]
        [SKParameter("vehicleType", "The type of vehicle")]
        [SKParameter("userName", "The name of the user")]
        public DataTable FetchAvailableParkingSlot(SKContext context)
        {
            ...
            using (var stream = File.Open("C:\\Users\\pramishra\\Desktop\\MLTrainingMaterial\\Semantic-Kernel-Workshop\\SKWorkshop\\Data\\SemanticKernel_ParkingData.xlsx", FileMode.Open, FileAccess.Read))
            {
                ...
                using (var reader = ExcelReaderFactory.CreateReader(stream))
                {
                    ...
                    string fuelType = context["vehicleType"];
